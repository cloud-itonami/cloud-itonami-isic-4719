(ns merchandiseops.phase
  "Phase 0->3 staged rollout for the ISIC-4719 'Other retail sale in
  non-specialized stores' (general-merchandise retail) operations-
  coordination actor.
  ...
  `:flag-loss-prevention-concern` is deliberately ABSENT from every
  phase's `:auto` set, including phase 3 -- a permanent structural fact,
  not a rollout milestone still to come. Flagging a loss-prevention
  concern always needs a human to actually look at it.
  `merchandiseops.governor`'s own `always-escalate-ops` enforces the same
  invariant independently -- two layers, not one, agree on this."
  (:require [merchandiseops.governor :as governor]))

(def read-ops #{})
(def write-ops governor/allowed-ops)

;; NOTE the invariant: `:flag-loss-prevention-concern` is a member of
...
(defn gate
  ...
  - `:flag-loss-prevention-concern` is never auto-eligible at any phase,
...
(defn verdict->disposition
  "Map a MerchandiseRetailGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
