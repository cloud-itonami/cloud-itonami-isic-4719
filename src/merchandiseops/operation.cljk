(ns merchandiseops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (MerchandiseRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the MerchandiseRetailGovernor
  ...
  human store operator/loss-prevention coordinator. The approver resumes
...
(defn build
  ...
  `merchandiseops.store/Store`).
  opts:
    :advisor      -- a `merchandiseops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MerchandiseRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; MerchandiseRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
