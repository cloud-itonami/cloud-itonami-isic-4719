(ns merchandiseops.governor-contract-test
...
(deftest loss-prevention-concern-always-escalates
  (testing ":flag-loss-prevention-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-2" :phase 3}
          result (exec-request actor "t2"
                               {:op :flag-loss-prevention-concern :store-id "store-1"
                                :patch {:concern "concealed merchandise" :confidence 0.99}}
                               ctx)]
      (is (some? result))
      ;; At this point the actor is paused for approval, not yet committed
      (is (= 0 (count (store/coordination-log db)))
          "loss-prevention concern must not auto-commit, must wait for approval")
      ;; Now approve it
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "display fixtures" :quantity 1 :estimated-cost 2600.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into loss-prevention-enforcement-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
